...
namespace pxsim {
      ...
      DBMeterBoard
  {
    ...
    constructor(public boardDefinition: BoardDefinition) {
      ...
      this.lightState = {};
      this.microphoneState = new MicrophoneState(
      ...
      this.lightSensorState = new AnalogSensorState(
      ...
      this.thermometerState = new AnalogSensorState(
      ...
      this.distanceState = new AnalogSensorState(
        DAL.DEVICE_ID_DISTANCE,
        0,
        2000
      );
      this.distanceUnitState = DistanceUnit.Millimeter;
      this.hygrometerState = new AnalogSensorState(
        DAL.DEVICE_ID_HUMIDITY,
        0,
        100
      );
      this.barometerState = new AnalogSensorState(
        DAL.DEVICE_ID_PRESSURE,
        980,
        1050
      );
      ...
      this.builtinParts["radio"] = this.radioState = new RadioState(
        runtime,
        this,
        {
      ...
      this.builtinParts["touch"] = this.touchButtonState = new TouchButtonState(
        pinList
      );
      ...
      this.builtinParts["screen"] = this.screenState = new ScreenState(
        [],
        getConfig(DAL.CFG_DISPLAY_WIDTH) || 160,
        getConfig(DAL.CFG_DISPLAY_HEIGHT) || 128
      );

      this.builtinVisuals["buttons"] = () => new visuals.ButtonView();
      this.builtinVisuals["microservo"] = () => new visuals.MicroServoView();

      this.builtinParts["neopixel"] = (pin: Pin) => {
        return this.neopixelState(pin.id);
      };
      this.builtinVisuals["neopixel"] = () =>
        new visuals.NeoPixelView(parsePinString);
      this.builtinPartVisuals["neopixel"] = (xy: visuals.Coord) =>
        visuals.mkNeoPixelPart(xy);

      this.builtinParts["dotstar"] = (pin: Pin) => {
        return this.neopixelState(pin.id);
      };
      this.builtinVisuals["dotstar"] = () =>
        new visuals.NeoPixelView(parsePinString);
      this.builtinPartVisuals["dotstar"] = (xy: visuals.Coord) =>
        visuals.mkNeoPixelPart(xy);

      this.builtinParts["lcd"] = this.lcdState;
      this.builtinVisuals["lcd"] = () => new visuals.LCD2View();
      this.builtinPartVisuals["lcd"] = (xy: visuals.Coord) =>
        visuals.mkLCD2Part(xy);

      this.builtinPartVisuals["buttons"] = (xy: visuals.Coord) =>
        visuals.mkBtnSvg(xy);
      ...
      this.builtinVisuals["slideswitch"] = () =>
        new visuals.ToggleComponentVisual(parsePinString);
      this.builtinPartVisuals["slideswitch"] = (xy: visuals.Coord) =>
        visuals.mkSideSwitchPart(xy);

      this.builtinParts["led"] = (pin: Pin) => new ToggleState(pin);
      this.builtinVisuals["led"] = () => new visuals.LedView(parsePinString);
      this.builtinPartVisuals["led"] = (xy: visuals.Coord) =>
        visuals.mkLedPart(xy);

      this.builtinVisuals["photocell"] = () =>
        new visuals.PhotoCellView(parsePinString);
      this.builtinPartVisuals["photocell"] = (xy: visuals.Coord) =>
        visuals.mkPhotoCellPart(xy);

      this.builtinVisuals["screen"] = () => new visuals.ScreenView();
      this.builtinPartVisuals["screen"] = (xy: visuals.Coord) =>
        visuals.mkScreenPart(xy);

      this.neopixelPin =
        this.edgeConnectorState.getPin(
          getConfig(DAL.CFG_PIN_ONBOARD_DOTSTAR_DATA)
        ) ||
        this.edgeConnectorState.getPin(
          getConfig(DAL.CFG_PIN_ONBOARD_NEOPIXEL)
        ) ||
      ...
      this.builtinParts["pixels"] = (pin: Pin) => {
        return this.neopixelState(!!this.neopixelPin && this.neopixelPin.id);
      };
      this.builtinVisuals["pixels"] = () =>
        new visuals.NeoPixelView(parsePinString);
      this.builtinPartVisuals["pixels"] = (xy: visuals.Coord) =>
        visuals.mkNeoPixelPart(xy);

      this.builtinParts["distance"] = () =>
        new DistanceState(this.distanceState, this.distanceUnitState);
      this.builtinVisuals["distance"] = () => new visuals.DistanceView();

      this.builtinParts["hygrometer"] = () =>
        new HygrometerState(this.hygrometerState);
      this.builtinVisuals["hygrometer"] = () => new visuals.HygrometerView();

      this.builtinParts["thermometer"] = () =>
        new ThermometerState(this.hygrometerState);
      this.builtinVisuals["thermometer"] = () => new visuals.ThermometerView();

      this.builtinParts["barometer"] = () =>
        new BarometerState(this.hygrometerState, this.pressureUnitState);
      ...
      this.builtinPartVisuals["ssd1306"] = (xy: visuals.Coord) =>
        visuals.mkSSD1306Part(xy);
      ...
      this.builtinPartVisuals["anemometer"] = (xy: visuals.Coord) =>
        visuals.mkAnemometer(xy);
